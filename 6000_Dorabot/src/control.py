#!/usr/bin/env python
"""
Modified by: blici@connect.ust.hk
Modified date: 2026-02
Changes: Add ctx to Queue.__init__().
"""
...
class RedirectQueue(Queue):
    def __init__(self,maxsize=0):
        ...
        super(RedirectQueue, self).__init__(maxsize=maxsize, ctx=ctx)
...
class ControlApp(Cmd):
    ...
    def listener_thread(self, in_q):
        def retrieve_title_from_pid(pid):
            for title, p_interface in list(self.children_process_dict.items()):
    ...
    def complete_new(self, text, line, begidx, endidx):
        flag_dict = \
            {
                '--title': list(self.children_process_dict.keys()),
    ...
    def do_quit(self, args):
        ...
        for interface in list(self.children_process_dict.values()):
    ...
    def complete_activate(self, text, line, begidx, endidx):
        index_dict = \
            {
                1: list(self.children_process_dict.keys())  # Tab-complete food items at index 1 in command line
            }
    ...
    def do_log(self, title):
        ...
        title = self.__process_title(title)
        if title: # found
            for t, p_interface in list(self.children_process_dict.items()):
                if t == title:
    ...
    def complete_print(self, text, line, begidx, endidx):
        index_dict = \
            {
                1: list(self.variable_dict.keys()),  # Tab-complete food items at index 1 in command line
            }
        return self.index_based_complete(text, line, begidx, endidx, index_dict=index_dict)
