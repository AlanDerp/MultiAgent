...
def controller_cmd_handler(simulator, running):
    ...
    while not simulator.receive_q.empty():
        ...
        if flag == SUBPROCESS_END_FLAG:
            ...
            if not cmd.ids:
                cmd.ids = list(range(len(simulator.agents)))
            valid_path = None
            for aid in cmd.ids:
                if cmd.path: # parse path
                    try: # in case of no valid tuple
                        coord_list = list([to_point(list(map(float, re.split('[ ]*,[ ]*',x)))) for x in re.findall(r'\((.*?)\)',cmd.path)])
...
def convert_planner_name_by_uppercase(name):
    segments = [x for x in re.split('([A-Z][a-z]*)', name) if len(x)>0]
    class_name = name
